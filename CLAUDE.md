...
- `geo.html`, `berater.html` and `verkaufsleitfaden.html` are standalone auxiliary pages, not part of the main nav flow. `verkaufsleitfaden.html` is linked from a `link-card` in `index.html`'s `#links` section instead.
